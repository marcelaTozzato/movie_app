...
class MoviesViewControllerMockingControllerForSuccess2Pages: MoviesController {
    
    override func loadMovies(sessionManager: SessionManager, page: Int) {
        ...
        do {
        let decodedObject: Movies = try JSONDecoder().decode(Movies.self, from: Data(mockedMovieReturn.utf8))
            super.sucessLoadMovie(movie: decodedObject)
        } catch {
